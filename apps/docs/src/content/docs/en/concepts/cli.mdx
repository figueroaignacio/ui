---
title: CLI
description: Use the MateUI CLI to initialize your project, add, update, remove, and list components directly from the terminal.
published: true
toc:
  visible: true
date: 2026-04-10
...
<Callout variant="info" className="mt-4">
  ...
  <Callout.Content>
    If `mateui.json` already exists, `init` will detect it and skip re-injecting tokens to avoid
    duplicates.
  </Callout.Content>
</Callout>
...
```bash
pnpm dlx mateui update <component>
```
...
```bash
pnpm dlx mateui remove <component>
```
...
```bash
# 1. Initialize MateUI in your project
pnpm dlx mateui init
